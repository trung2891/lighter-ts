  ...
  try {
    ...
    for (const marketId of marketsToCheck) {
      try {
        ...
        const orders = Array.isArray(activeOrders)
          ? activeOrders
          : (activeOrders as any).orders || [];
    ...
    allOrders.forEach((order: any, index: number) => {
      console.log(
        `   ${index + 1}. Market ${order.market_id || order.marketId}: ${order.side} ${order.type} - Size: ${order.size || order.base_amount} - Price: ${order.price}`
      );
    });
    console.log();
    ...
    const [tx, txResult, error] = await signerClient.cancelAllOrders(0, 0);
